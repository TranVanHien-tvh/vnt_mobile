<template>
  <div class="ms-combobox ms-editor">
    ...
    >
      ...
      <div
        class="icon-combo--dropdown display"
...
<script>
import MsComboBoxDropdown from "@/components/mscombobox/MsComboBoxDropdown.vue";
import MsComboBoxItem from "@/components/mscombobox/MsComboBoxItem.vue";
  ...
  methods: {
    ...
    setInitText(val) {
      ...
      if (me.internalDataSource.length > 0) {
        var selected = me.internalDataSource.filter(function (item) {
          return item[me.displayField] == me.internalText;
        });
        if (selected.length > 0) {
          me.internalValue = selected[0][me.valueField];
          me.internalSelectedItem = selected[0];
        }
      }
    },
    /**
