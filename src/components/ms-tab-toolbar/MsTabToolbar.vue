  ...
  >
    <div class="con-ms-ul-tabs">
      ...
      >
        ...
        >
          <div class="ms-tabs-btn" type="button" @click="activeItem(index)">
            <label class="tab-label"
              >{{ item.title }}
              <span v-if="item.beta == true" class="beta">beta</span>
            </label>
          </div>
        </li>
      </ul>
  ...
  methods: {
        ...
        if (Array.isArray(tab.matchPath)) {
          for (let j = 0; j < tab.matchPath.length; j++) {
            for (let k = 0; k < routers.length; k++) {
              if (
                routers[k].path === tab.matchPath[j] ||
    ...
    getIndexItem(currentPath) {
      ...
      for (var i = 0; i < this.tabs.length; i++) {
        if (this.tabs[i].path.toUpperCase().indexOf(upper) > -1) {
          return i;
        }
      }
    },
    ...
    async activeItem(index) {
      ...
      if (this.tabs[index].path !== "") {
        ...
        if (
          this.$route.path.toLowerCase() !== this.tabs[index].path.toLowerCase()
        ) {
...
@import "@/assets/scss/components/msTabToolbar.scss";
</style>
