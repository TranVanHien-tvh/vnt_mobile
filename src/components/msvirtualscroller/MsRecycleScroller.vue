<template>
  ...
  >
    ...
    <div
      ref="wrapper"
      :style="{ [direction === 'vertical' ? 'minHeight' : 'minWidth']: totalSize + 'px','minWidth':`${minWidth}px`}"
...
export default {
	...
	methods: {
		...
		updateVisibleItems(checkItem) {
			...
			// //PDKIEn-Check lần đầu xổ
			// if (!this.firstRun) {
			// 	this.firstRun = true;
			// 	if (this.visiableIndex > 0) {
			// 		let me = this;
			// 		setTimeout(() => {
			// 			me.scrollToItem(this.visiableIndex);
			// 		}, 10);
			// 	}
			// 	return;
			// }
		...
		calculateScrollPosition(index, anchorTop) {
			let scroll, min, state;
			if (anchorTop) {
				if (this.itemSize === null) {
				...
				if (this.itemSize === null) {
					...
					scroll = index * this.itemSize;
					min = scroll - this.$el.clientHeight;
					//state = this.getScroll();
					// if (state.start <= scroll && scroll <= state.end) {
					// 	scroll = -1;
					// } else {
					// 	scroll = min + this.itemSize;
					// }
					scroll = min + this.itemSize;
				}
				return scroll;
		...
		scrollToItem(index, anchorTop) {
			this.scrollToPosition(this.calculateScrollPosition(index, anchorTop));
		},
...
.dropdown-items .vue-recycle-scroller__item-view {
	display: table;
}
