<template>
  ...
  >
    <div class="combobox-item-con">
      ...
      <div
        v-if="(item.multiSelected || isSelected) && isShowCheckBox"
  ...
  >
    ...
    <div
      v-if="item.multiSelected || isSelected"
      class="selected-container"
    >
      <div :class="['icon24','mi-checkbox-active']" />
    </div>
...
export default {
	...
	props: {
		...
		/**
		 * có muốn show tích chọn hay không
		 * DLHuy 01.03.2022
		 */
		isShowCheckBox:{
			type: Boolean,
	...
	methods: {
		 ...
		 */
		getCellStyle(column, index) {
			if (column.isResize) {
				if (this.$refs[`td-${index}`] && this.$refs[`td-${index}`].length > 0) {
					let item = this.$refs[`td-${index}`][0];
					let rectTd = item.getBoundingClientRect();
					let rectParent = this.$parent.$el.getBoundingClientRect();
